...
!--------------------------------------------------------------
 ...
 if (enable_cyclic_y) then
    if (n_pes_j == 1) then
      ...
      tag=1
      do j=1,onx
        if (my_blk_j == 1       ) call mpi_send(p1(:,j   ),len,mpi_real8,my_pe+n_pes_i*(n_pes_j-1),tag,my_comm,ierr)
        if (my_blk_j == n_pes_j ) call mpi_recv(p1(:,ny+j),len,mpi_real8,my_pe-n_pes_i*(n_pes_j-1),tag,my_comm,status,ierr)
      enddo
      tag=2
      do j=1,onx
        if (my_blk_j == n_pes_j ) call mpi_send(p1(:,ny-j+1),len,mpi_real8,my_pe-n_pes_i*(n_pes_j-1),tag,my_comm,ierr)
        if (my_blk_j == 1 )       call mpi_recv(p1(:,1-j   ),len,mpi_real8,my_pe+n_pes_i*(n_pes_j-1),tag,my_comm,status,ierr)
      enddo
    endif
 endif
...
!--------------------------------------------------------------
  ...
  if (enable_cyclic_y) then
    if (n_pes_j == 1) then
      ...
      tag=1
      do j=1,onx
        if (my_blk_j == 1       ) call mpi_send(p1(:,j   ),len,mpi_integer,my_pe+n_pes_i*(n_pes_j-1),tag,my_comm,ierr)
        if (my_blk_j == n_pes_j ) call mpi_recv(p1(:,ny+j),len,mpi_integer,my_pe-n_pes_i*(n_pes_j-1),tag,my_comm,status,ierr)
      enddo
      tag=2
      do j=1,onx
        if (my_blk_j == n_pes_j ) call mpi_send(p1(:,ny-j+1),len,mpi_integer,my_pe-n_pes_i*(n_pes_j-1),tag,my_comm,ierr)
        if (my_blk_j == 1 )       call mpi_recv(p1(:,1-j   ),len,mpi_integer,my_pe+n_pes_i*(n_pes_j-1),tag,my_comm,status,ierr)
      enddo
    endif
 endif
